<template>
    <div class="app-container product-container">
        <el-row class="app-query">
            ...
            <el-select clearable style="width: 150px"  v-model="listQuery.boilerModelNumber" placeholder="锅炉型号">
...
<script>
    ...
    import {getProductListByCondition,deleteProductById,getProductUserListByProductCondition,insertManyProductUser} from '@/api/product'
    ...
    export default {
        ...
        created() {
            Promise.all([this.initSelect()]).then(()=>{this.getList()})
        },
        methods: {
            initSelect(){
            ...
            initAuxiliaryMachineAbout(){
                return new Promise((resolve, reject)=>{
                    getAuxiliaryMachineLargeClassListByCondition({}).then(response=>{
                        this.largeClassArray=this.getAuxiliaryMachineAboutOptions(response.data.data)
                    })
                    getAuxiliaryMachineSmallClassListByCondition({}).then(response=>{
                        this.smallClassArray=this.getAuxiliaryMachineAboutOptions(response.data.data)
                    })
                    resolve()
                });
            },
            ...
            handleCreate() {
                ...
                let newWindow=openElectronWindow("/product-form?title=create",{width: width, height: height})
            ...
            handleUpdate(row) {
                ...
                let newWindow=openElectronWindow("/product-form?title=edit&&productFormData="+JSON.stringify(row),{width: width, height: height})
                newWindow.on('closed', () => {newWindow = null})
                ipcMain.once('handleCloseProductForm', (event, arg) => {
                    newWindow.close()
                    newWindow = null
            ...
            showMap(){
                ...
                let newWindow=openElectronWindow("/map-complete-page",{width: width, height: height})
                newWindow.on('closed', () => {
                    newWindow = null
                })
            },
            sellProduct(row){
                let width= Math.round(document.body.clientWidth/2)+175
                let height= Math.round(document.body.clientHeight/2)+175
                let newWindow=openElectronWindow("/product-map?productMapData="+JSON.stringify(row),{width: width, height: height})
                newWindow.on('closed', () => {
                    this.getList()
                    newWindow = null
                })
            ...
            baseInfoInfo(row){
                ...
                let newWindow=openElectronWindow("/base-run-info-complete-page?controllerNo="+row.controllerNo,{width: width, height: height})
                newWindow.on('closed', () => {
                    newWindow = null
                })
            },
            handleChoiceUser(row){
                ...
                getUserListByCondition({status:statusManage.STATUS_ENABLE,orgId:this.$store.state.user.orgId,orgType:this.$store.state.user.orgType}).then(response => {
                    let userArray=response.data.data;
                    this.choiceUserFormData.userArray=userArray
                    userArray.forEach(item=>{
                        if(item.id!=this.$store.state.user.userId) this.choiceUserFormData.userOptions.push({value:item.id,label:item.realName})
                    })
                })
                getProductUserListByProductCondition({productId:row.id}).then(data=>{
                    let userIdArray=[]
                    data.data.data.forEach(item=>{
                        if(item.userId!=this.$store.state.user.userId) userIdArray.push(item.userId)
                    })
            ...
            confirmSubmitChoiceUser(){
                ...
                this.choiceUserFormData.deleteUserIdArray.forEach(userId=>{
                    deleteProductUserList.push({userId:userId,productId:this.choiceUserFormData.productId})
                })
                let selectProductUserList=[]
                this.choiceUserFormData.selectUserIdArray.forEach(userId=>{
                    selectProductUserList.push({userId:userId,productId:this.choiceUserFormData.productId})
                })
                insertManyProductUser({deleteProductUserList:deleteProductUserList,selectProductUserList:selectProductUserList}).then(data=>{
            ...
            handleDownload(row) {
                this.initAuxiliaryMachineAbout().then(()=>{
                    return getProductAuxiliaryMachineInfoListByProductId({productId:row.id})
                }).then(response=>{
                    this.productAuxiliaryMachineInfoList=response.data.data
                    return import('./exportToExcel')
                }).then(excel=>{
                    let excelObjArray=[]
                    const boiler_header = ['锅炉编号','锅炉型号', '控制器编号', '燃料','介质','吨位（T）','是否售出','客户名称','出售日期','出售地址']
                    const boiler_filter_val = ['boilerNo', 'boilerModelNumber', 'controllerNo', 'fuel','medium','tonnageNum','isSell', 'boilerCustomerName','saleDate','saleAddress']
                    const boiler_data={
                        boilerNo:row.boilerNo,
                        boilerModelNumber:(()=>{
                            let dictionary=dictionaryValueFilter(this.boilerModelNumberArray,row.boilerModelNumber)
                        ...
                        tonnageNum:row.tonnageNum,
                        medium:(()=>{
                            let dictionary=dictionaryValueFilter(this.mediumArray,row.medium)
                            if(dictionary) return dictionary.label
                            return ""
                        })(),
                        fuel:(()=>{
                        ...
                        isSell:(()=>{
                            let dictionary=dictionaryValueFilter(this.isSellArray,row.isSell)
                            if(dictionary) return dictionary.label
                            return ""
                        })(),
                        boilerCustomerName:(()=>{
                    ...
                    if(this.productAuxiliaryMachineInfoList.length>0){
                        const productAuxiliaryMachineInfo_header=['大类','小类','品牌','型号','数量','供货厂家','备注']
                        const productAuxiliaryMachineInfo_filter_val=['largeClassName','smallClassName','brandName','modelName','amountOfUser','supplier','remarks']
                        this.productAuxiliaryMachineInfoList.forEach(item=>{
                            item.largeClassName=(()=>{
                                let dictionary=dictionaryValueFilter(this.largeClassArray,item.largeClassId)
                                if(dictionary) return dictionary.label
                                return ""
                            })()
                            item.smallClassName=(()=>{
                                let dictionary=dictionaryValueFilter(this.smallClassArray,item.smallClassId)
                                if(dictionary) return dictionary.label
                                return ""
                            })()
                        })
                        excelObjArray.push({
                            ...
                            data:this.formatJson(productAuxiliaryMachineInfo_filter_val, this.productAuxiliaryMachineInfoList)
                        })
                    }
